...
<%--
    ...
    2 - send and receive image in messaging
    3 - check out button
    4 - messages button in navebar to show all messages with other.users
    - html-css for messages must be improved
    - add edit button on offer card
    - user profile
        show all this.user informaition
        edit all this.user informaition
        show all offers from this.user
        show all messages with other.users
    - english to german
--%>
...
<div class="container">
    ...
    <div class="row">
        <%
            ...
            List<ProductEntity> products = productRepo.getAllProducts();
            if (!products.isEmpty()) {
        ...
        <div class="col-md-3 my-3">
            <div class="card w-100">
                ...
                <div class="card-body">
                    ...
                    <h6 class="price">Price: <%=productEntity.getPreis() %> $
                    </h6>
                    <h6 class="price">Owner name: <%=productEntity.getOwner_name() %>
                    </h6>
                    <div class="mt-3 d-flex justify-content-between">

                        <%--                        <a class="btn btn-primary" href="order-now?anzahl=1&pk=<%=productEntity.getPk()%>">Buy Now</a>--%>
                        <%
                            if (!productEntity.getOwner_name().equals(session.getAttribute("name"))) {
                                // Angebot von anderem Benutzer
                        %>
                        <a class="btn btn-dark" href="add-to-cart?pk=<%=productEntity.getPk()%>">
                            <%--                            Add to Cart--%>
                        ...
                        <a class="btn btn-dark" href="message-to-user?owner_name=<%=productEntity.getOwner_name()%>">
                            <%--                            message to user --%>
                        ...
                        <%
                            ...
                            // Angebot von this Benutzer
                        %>
                        <a class="btn btn-primary" href="deleteOffer?pk=<%=productEntity.getPk()%>">
                            <%--                            delete offer--%>
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor"
